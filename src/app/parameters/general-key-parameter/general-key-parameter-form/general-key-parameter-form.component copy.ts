...
export class GeneralKeyParameterFormComponent implements OnInit {
  ...
  constructor(public routerActive: ActivatedRoute, private service: GeneralKeyParameterService, private helperService: HelperService, private modalActive: NgbActiveModal) { 
  ...
  ngOnInit(): void {
    if (this.id != undefined && this.id != null) {
      ...
      this.service.getById(this.serviceName, this.id).subscribe(l => {
        ...
        this.frmGeneralKey.controls.ForaneaId.setValue(l.data[this.foreignKeyRuta+"Id"]);
      })
    }else {
  ...
  save() {
      this.frmGeneralKey.get('ForaneaId')?.touched
      let a = this.frmGeneralKey.get('ForaneaId')?.touched
      a = true  
    ...
    // let data  = { 
    //   id: this.id ?? 0,
    //   [this.foreignKeyRuta+"Id"]: this.frmGeneralKey.controls.ForaneaId.value,
